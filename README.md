...
Текущая версия кода: **0.702**. Python **3.10+**, только стандартная библиотека.
...
MutaBasic - небольшой QBasic-подобный интерпретатор с необычным центром
тяжести: программа может читать и менять собственный нумерованный листинг.
Вместо попытки воспроизвести весь QBasic он делает компактное ядро, которое
удобно наблюдать, тестировать и встраивать в Python.

**Что уже хорошо получается:**

- интерактивное выполнение через REPL, пошаговый режим, точки останова,
  снимки, контрольные точки и JSON-совместимый trace;
- классические управляющие конструкции, массивы, `DATA`/`READ`, файлы,
  пользовательские `SUB`/`FUNCTION` и безопасная для рекурсии локальность
  параметров;
- транзакционное редактирование листинга, история `UNDO`/`REDO`, версии
  листинга и диагностика переходов через активные блоки;
- практичные строковые функции и единый поиск по исходнику и значениям;
- небольшой Python API без сторонних зависимостей, пригодный для тестовых
  стендов и учебных инструментов.

**Где нужны реалистичные ожидания:**

- это не полный QBasic и не средство запуска старых коммерческих `.BAS`;
  отсутствуют многие конструкции вроде `TYPE`, `SELECT CASE`, `ON ERROR`,
  графики, бинарных и random-файлов;
- параметры процедур передаются по значению, явных локальных объявлений и
  полноценной передачи по ссылке нет; не-параметрические переменные процедур
  остаются глобальными ради совместимости;
- `SOURCE` проверяет структуру и активные контексты, но не является полной
  статической проверкой программы;
- trace воспроизводим как журнал шагов при детерминированных входах, но это
  не полноценный record/replay и он не отменяет файловые, shell- или иные
  внешние эффекты;
- запуск BASIC не является песочницей: доступ к файлам и окружению требует
  отдельной изоляции процесса.

**Уникально полезные сценарии:**

1. Лаборатории по интерпретаторам и языкам: видно адреса инструкций, пары
   блоков, стек и изменения исходника, а Python API упрощает эксперименты.
2. Обучение отладке и исполнению: можно менять алгоритм во время остановки,
   сравнивать trace до/после и сохранять снимок состояния.
3. Генеративные и эволюционные эксперименты с кодом: `SOURCE` дает
   контролируемую мутацию нумерованной программы с атомарным откатом.
4. Тестовые стенды для DSL и REPL-прототипов: минимальная стандартная
   библиотека, встроенная история и детерминированные провайдеры времени и
   случайности позволяют быстро строить проверяемые сценарии.
5. Интерактивные демонстрации legacy-стиля программирования, где важны
   листинг, состояние и пошаговое наблюдение, а не производительность.

Для обычных приложений, строгой типизации, многопоточности, высокой
производительности или запуска недоверенного кода лучше выбрать другой
язык и среду.
...
`--help` выводит назначение всех ключей, примеры запуска и список тем
интерактивной справки. Он не запускает REPL и завершается с кодом `0`.
Для справки внутри REPL используйте:

```bash
python mutabasic.py -e "HELP"
python mutabasic.py -e "HELP commands"
```

Если указано несколько источников программы (`program`, `--project`,
`--restore`), CLI завершится с ошибкой; выберите ровно один источник.
