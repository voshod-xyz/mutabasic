...
Один номер удаляет строку:
  10
Номера строк должны быть положительными и уникальными.
...
Правила:
  ...
  * Номера строк должны быть положительными; повторные номера запрещены.
""",
    "system": """
...
Если задан хотя бы один --allow-shell-command, выполняются только
перечисленные executable. Цепочки (&, |), перенаправления, подстановки
и другие операторы оболочки при этом запрещены.
Без --allow-shell-command после --allow-shell сохраняется unrestricted-режим.
